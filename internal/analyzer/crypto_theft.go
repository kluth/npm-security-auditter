...
func (a *CryptoTheftAnalyzer) detectSignals(content string) cryptoSignals {
...
func (a *CryptoTheftAnalyzer) scanContent(content string, filename string) []Finding {
	s := a.detectSignals(content)
	var findings []Finding
	findings = append(findings, a.checkWalletAddresses(s, content, filename)...)
	findings = append(findings, a.checkClipboardHijack(s, content, filename)...)
	findings = append(findings, a.checkWalletFileTheft(s, content, filename)...)
	findings = append(findings, a.checkSeedPhraseTheft(s, content, filename)...)
	findings = append(findings, a.checkPrivateKeyTheft(s, content, filename)...)
	return findings
}

func (a *CryptoTheftAnalyzer) checkWalletAddresses(s cryptoSignals, content, filename string) []Finding {
	if !s.hasEthAddr && !s.hasBtcAddr {
		return nil
	}
	addrTypes := []string{}
	if s.hasEthAddr {
...
func (a *CryptoTheftAnalyzer) checkClipboardHijack(s cryptoSignals, content, filename string) []Finding {
	if !s.hasClipboard || !(s.hasEthAddr || s.hasBtcAddr || strings.Contains(content, "0x[a-fA-F") || strings.Contains(content, "match(/")) {
		return nil
	}
	return []Finding{{
		Analyzer:    a.Name(),
...
func (a *CryptoTheftAnalyzer) checkWalletFileTheft(s cryptoSignals, content, filename string) []Finding {
	if !s.hasWalletPath || !(s.hasExfil || strings.Contains(content, "readFile") || strings.Contains(content, "readFileSync")) {
		return nil
	}
	return []Finding{{
		Analyzer:    a.Name(),
...
func (a *CryptoTheftAnalyzer) checkSeedPhraseTheft(s cryptoSignals, content, filename string) []Finding {
	if !s.hasSeedPhrase || !(s.hasExfil || strings.Contains(content, "addEventListener")) {
		return nil
	}
	return []Finding{{
		Analyzer:    a.Name(),
...
func (a *CryptoTheftAnalyzer) checkPrivateKeyTheft(s cryptoSignals, content, filename string) []Finding {
	if !s.hasPrivateKey || !(s.hasExfil || strings.Contains(content, "readFile")) {
		return nil
	}
	return []Finding{{
		Analyzer:    a.Name(),
