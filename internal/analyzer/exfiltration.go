...
// isExfiltrationURL checks if a URL matches known exfiltration patterns.
func isExfiltrationURL(rawURL string) (bool, string) {
	...
	for _, endpoint := range exfiltrationEndpoints {
		if endpoint.Pattern.MatchString(rawURL) {
			return true, endpoint.Name
		}
	}

	// Check for DGA-like domains (high entropy hostnames)
	if len(host) > 0 {
		parts := strings.Split(host, ".")
		if len(parts) >= 2 {
			domain := parts[len(parts)-2]
			if len(domain) > 10 && hasHighEntropy(domain) {
				return true, "DGA-like domain"
			}
		}
	}
...
// hasHighEntropy checks if a string has high character entropy (DGA indicator).
func hasHighEntropy(s string) bool {
	...
	// Also check for consonant clusters (common in DGA)
	consonants := 0
	maxConsonantRun := 0
	currentRun := 0
	vowels := "aeiou"
	...
	for _, c := range strings.ToLower(s) {
		if c >= 'a' && c <= 'z' && !strings.ContainsRune(vowels, c) {
			consonants++
			currentRun++
	...
	consonantRatio := float64(consonants) / float64(len(s))

	// DGA domains typically have: high unique ratio, high consonant ratio, long consonant runs
	return uniqueRatio > 0.6 && consonantRatio > 0.7 && maxConsonantRun >= 4
}
